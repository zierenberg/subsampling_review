...
def main():
    ...
    filename="/{}/data/data_avalanche_sandpile.h5".format(base_path)
    f = h5py.File(filename,'r')

    # random subsampling
    ax = axs[0]
    plot_data = np.transpose(f["size"]["full"])
    ax.plot(plot_data[:,0], plot_data[:,1], '-', color='black', label=r"$\left(\frac{128}{128}\right)^2=100\%$")

    plot_data = np.transpose(f["size"]["random"]["64"])
    ax.plot(plot_data[:,0], plot_data[:,1], '-', color= _alpha_to_solid_on_bg('blue', 1.0), label=r"$\left(\frac{64}{128}\right)^2=25\%$")

    plot_data = np.transpose(f["size"]["random"]["16"])
    ax.plot(plot_data[:,0], plot_data[:,1], '-', color= _alpha_to_solid_on_bg('blue', 0.7), label=r"$\left(\frac{16}{128}\right)^2\approx 1.6\%$")

    plot_data = np.transpose(f["size"]["random"]["4"])
    ax.plot(plot_data[:,0], plot_data[:,1], '-', color= _alpha_to_solid_on_bg('blue', 0.3), label=r"$\left(\frac{4}{128}\right)^2\approx 0.1\%$")
    ...
    ax = axs[1]
    plot_data = np.transpose(f["size"]["full"])
    ax.plot(plot_data[:,0], plot_data[:,1], '-', color='black', label=r"$128 \times 128$")

    plot_data = np.transpose(f["size"]["window"]["64"])
    ax.plot(plot_data[:,0], plot_data[:,1], '-', color=_alpha_to_solid_on_bg('orange', 1.0), label=r"$64\, \times 64$")

    plot_data = np.transpose(f["size"]["window"]["16"])
    ax.plot(plot_data[:,0], plot_data[:,1], '-', color=_alpha_to_solid_on_bg('orange', 0.7), label=r"$16 \times 16$")

    plot_data = np.transpose(f["size"]["window"]["4"])
    ax.plot(plot_data[:,0], plot_data[:,1], '-', color=_alpha_to_solid_on_bg('orange', 0.3), label=r"$4 \times 4$")
    ...
    #plt.show()
    plt.savefig("/{}/plots/avalanche_sandpile.pdf".format(base_path))
